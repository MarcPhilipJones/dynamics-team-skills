...
| [flows-via-web-api](power-platform/flows-via-web-api/SKILL.md) | "create a flow via API", "POST /workflows", "flow won't open in the designer", "trigger fires on wrong change type", "API flow won't fire" | Create Dataverse-triggered cloud flows programmatically + gotchas (message codes, schemaVersion, operationMetadataId, trigger conditions, webhook re-registration/portal toggle, cloning Skills flows, team owner role) |
| [ccaas-outbound-voice-agent](power-platform/ccaas-outbound-voice-agent/SKILL.md) | "AI voice agent to call leads", "outbound qualification calls", "proactive voice", "Copilot Studio voice agent over a phone number", "voice agent won't publish", "which voice / caller ID / call recording" | Outbound AI voice qualification: CCaaS proactive voice + Copilot Studio agent (pac CLI) + orchestration & write-back. Incl. deterministic scripted variant, pac-clone/publish gotchas, Classic vs Realtime voice, caller-ID, recording/transcript, conversation→lead linkage |
| [ai-builder-prompt-in-flow](power-platform/ai-builder-prompt-in-flow/SKILL.md) | "score with AI instead of a formula", "replace Power Fx with an AI prompt", "run an AI Builder prompt in a flow", "GPT to qualify inside Power Automate" | Wire an AI Builder custom prompt into a Dataverse/agent flow to return structured JSON (score/band/rationale) and parse it reliably — replacing brittle Power Fx logic |
| [build-intents](power-platform/build-intents/SKILL.md) | "add intents", "build intents", "set up the Customer Intent Agent", "create an intent library", "populate intents for a demo", "add intent groups" | Interview-driven pre-sales build of Customer Intent Agent groups + intents + one follow-up attribute each via Dataverse MCP; covers the name/intentstring auto-overwrite gotcha and the manual All-Users assignment |
| [link-kb-to-intents](power-platform/link-kb-to-intents/SKILL.md) | "link KB articles to intents", "fill the intent Knowledge articles tab", "map KB to intents", "attach articles to the Customer Intent Agent" | Link published knowledge articles to intents via msdyn_intentsolutionmap (the intent Knowledge articles tab); covers solutiontype=knowledgearticle, using the ROOT article id, and reusing the parent group's condition |
